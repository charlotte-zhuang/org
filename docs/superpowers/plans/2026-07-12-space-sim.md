...
**One deviation from the spec, decided here:** the spec says stars and particles are exposed as flat `Float32Array`s like the dots. Dots (thousands, the hot path) do use `Float32Array`s; stars (≤3) and particles (≤60) are pooled plain objects because at that count clarity beats layout micro-optimization, and the spec's real requirements — dumb-loop rendering and zero per-frame allocation — hold either way. A second micro-deviation: the spec's rendering section says to draw in CSS units via `ctx.scale(dpr, dpr)`; the implementation instead multiplies coordinates by `dpr` and rounds per-rect, because true device-pixel snapping requires post-scale rounding anyway.
