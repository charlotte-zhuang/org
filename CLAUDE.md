...
- The visible page background is a fixed full-viewport `-z-20 bg-background` div in `app/layout.tsx` (behind the `-z-10` canvas). This is deliberate, not decoration: `<body>` is shorter than the viewport, and WebKit fails to repaint a body background that has propagated to the root canvas surface when the theme class flips (stale until reload, Safari/iOS only — <https://developer.apple.com/forums/thread/734135>). Element boxes repaint correctly, so the page background must always be painted by a real full-viewport element — don't remove the backdrop div in favor of body/html backgrounds.
