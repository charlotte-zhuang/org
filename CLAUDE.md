...
The header's interactive title starts with "charlotte zhuang", deletes the current suffix, and types a randomly selected different suffix. After a reveal completes, that suffix becomes the next cycle's starting point. Pointer-out reverses an in-flight animation; a completed cycle has already reset internally and remains on its revealed suffix. The implementation is deliberately split so the animation logic is framework-agnostic and testable:

- `lib/title-typewriter.ts` — `TitleTypewriter`, a plain class with no React dependency. It accepts a `[string, string, ...string[]]` suffix tuple, uses the first suffix initially, and randomly chooses a different destination for each cycle. The whole effect is modeled as a single integer `progress` walking a fixed path (`0` = current suffix, `total` = revealed suffix); the displayed string is a pure function of `progress`. `play()`/`reverse()` only set a target, so flipping the target mid-flight redirects from the current position. On completion, the revealed suffix is promoted to the current suffix and `progress`/`target` reset to `0`. Runtime inputs with fewer than two distinct suffixes are inert. Emits each change through a single `onText` callback; behavior is covered by `lib/title-typewriter.test.ts`.
